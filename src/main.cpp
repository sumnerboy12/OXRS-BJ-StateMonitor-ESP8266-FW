/**
  ESP8266 state monitor firmware for the Open eXtensible Rack System
  ...
  GitHub repository:
    https://github.com/sumnerboy12/OXRS-BJ-StateMonitor-ESP8266-FW

  Copyright 2019-2022 Ben Jones
*/
...
uint8_t parseInputType(const char * inputType)
{
  ...
  room8266.println(F("[smon] invalid input type"));
...
uint8_t getIndex(JsonVariant json)
{
  ...
  {
    room8266.println(F("[smon] missing index"));
    return 0;
  }
  ...
  {
    room8266.println(F("[smon] invalid index"));
    return 0;
  }
...
void publishEvent(uint8_t index, uint8_t type, uint8_t state)
{
  ...
  if (!room8266.publishStatus(json.as<JsonVariant>()))
  {
    room8266.print(F("[smon] [failover] "));
    serializeJson(json, room8266);
    room8266.println();
...
void scanI2CBus()
{
  room8266.println(F("[smon] scanning for I/O buffers..."));

  for (uint8_t mcp = 0; mcp < MCP_COUNT; mcp++)
  {
    room8266.print(F(" - 0x"));
    room8266.print(MCP_I2C_ADDRESS[mcp], HEX);
    room8266.print(F("..."));
    ...
    if (Wire.endTransmission() == 0)
    {
      ...
      room8266.print(F("MCP23017"));
      if (MCP_INTERNAL_PULLUPS) { room8266.print(F(" (internal pullups)")); }
      room8266.println();
    }
    else
    {
      room8266.println(F("empty"));
    }
  }
}
